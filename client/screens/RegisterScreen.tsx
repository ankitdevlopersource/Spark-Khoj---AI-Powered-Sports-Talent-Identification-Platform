...
const RegisterScreen: React.FC<RegisterScreenProps> = ({ onRegister, onNavigateToLogin }) => {
  const [formData, setFormData] = useState({
    ...
    district: '',
    profilePictureUrl: 'https://picsum.photos/seed/default/100/100', // This is a base64 string
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        onNavigateToLogin(); // Navigate to login screen on success
    ...
    <div className="flex flex-col bg-gray-50 text-gray-800 min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col flex-1">
        <main className="flex-1 p-6 overflow-y-auto space-y-4">
          ...
          <input type="password" placeholder="Password" name="password" value={formData.password} onChange={handleInputChange} className="w-full px-4 py-3 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required />
       ...
       <div className="p-6">
         <button type="submit" disabled={isLoading} className="w-full py-3 bg-blue-800 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-lg disabled:bg-blue-900 disabled:cursor-not-allowed">
